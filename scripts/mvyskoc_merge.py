...
def mvyskoc_merge(subs_a_filename, sub_b_filename, out_filename):
    ...
    subs_a = SubRipFile.open(subs_a_filename, encoding=encoding)
    subs_b = SubRipFile.open(sub_b_filename, encoding=encoding)
    out = merge_subtitle(subs_a, subs_b, delta)
    out.save(out_filename, encoding=encoding)
